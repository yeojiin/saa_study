...
- 특정 구조가 있는 기업 데이터 센터를 AWS와 비공개로 연결하려면 기업은 고객 게이트웨이를 VPC는 VPN 게이트웨 필요
- 공용 인터넷을 통해 사설 Site-to-Site VPN을 연결
- VPN 연결이라서 암호화
-  VPN은 가상 프라이빗 게이트웨이 즉 VGW가 필요
- VPN 연결에서 AWS 측에 있는 VPN 집선기(concentrator)이며 VPC와 연결됨
- Site-to-Site VPN 연결을 생성하려는 VPC
- ASN을 지정할 수도 있음
- CGW는 고객 게이트웨이
- 소프트웨어 혹은 물리적 장치로 VPN 연결에서 데이터 센터
- 고객 게이트웨이가 있는 기업 데이터 센터와 가상 프라이빗 게이트웨이를 갖춘 VPC가 있을 때 온프레미스 고객 게이트웨이를 어떻게 구축해야 할까?
- 고객 게이트웨이가 공용이라면 인터넷 라우팅이 가능한 IP 주소가 고객 게이트웨이 장치에 있음, 이걸 사용해서 VGW와 CGW를 연결하면 됨 -> 고객 게이트웨이의 공용 IP를 사용
- 고객 게이트웨이를 비공개로 남겨 사설 IP를 가질 수도 있음
- NAT-T를 활성화하는 NAT 장치 뒤에 있음
- NAT 장치에 공용 IP가 있을 시 이 공용 IP를 CGW에 사용
- Site-to-Site VPN 연결은 이렇게 생성
- 서브넷의 VPC에서 라우트 전파를 활성화해야 Site-to-Site VPN 연결이 실제로 작동
- 온프레미스에서 AWS로 EC2 인스턴스 상태를 진단할 때 보안 그룹 인바운드 ICMP 프로토콜이 활성화됐는지 확인해야 함 -> 그렇지 않음 연결 안 됨
- Site-to-Site VPN에서 AWS VPN CloudHub도 중요!
...
- Direct Connect(DX)는 원격 네트워크로부터 VPC로의 전용 프라이빗 연결을 제공
- Direct Connect를 사용할 때는 전용 연결을 생성해야 하고, AWS Direct Connect 로케이션을 사용
- VPC에는 가상 프라이빗 게이트웨이를 설치해야 온프레미스 데이터 센터와 AWS 간 연결이 가능
- Amazon S3 등의 퍼블릭 리소스와 EC2 인스턴스 등의 프라이빗 리소스에 퍼블릭 및 프라이빗 VIF를 사용해 액세스할 수 있음
...
- 하이브리드 환경을 지원해 온프레미스 데이터 센터와 클라우드가 연결되어 있기 때문
- IPv4와 IPv6 둘 다 지원
- 리전을 기업 데이터 센터에 연결하려면 AWS Direct Connect 로케이션을 요청
- Direct Connect 엔드포인트가 있고, 고객이나 파트너 케이지(Cage)에서 빌려 와야 하는 고객 혹은 파트너 라우터도 이렇게 있고, Direct Connect 로케이션에 케이지 두 개가 있고, 온프레미스 데이터 센터에는 방화벽이 있는 고객 라우터를 설치
- 프라이빗 가상 인터페이스 즉 프라이빗 VIF를 생성하여 VPC로 프라이빗 리소스에 액세스
- 모든 로케이션을 연결하는 프라이빗 VIF를 생성하여 가상 프라이빗 게이트웨이에 연결해야 함
- 가 상 프라이빗 게이트웨이는 VPC에 연결되어 있음
- 프라이빗 VIF를 통해서 EC2 인스턴스가 있는 프라이빗 서브넷에 액세스할 수 있음
- 수동으로 연결해야 하므로 설치만 기간이 오래 걸릴 수 있음
- 퍼블릭 인터넷을 전혀 지나지 않고, 전부 비공개로 연결됨
- AWS 내 퍼블릭 서비스 Amazon Glacier 또는 Amazon S3에 연결할 수도 있는데 퍼블릭 가상 인터페이스 즉 퍼블릭 VIF를 설치해야 함
- 가상 프라이빗 게이트웨이로 연결되지 않고, AWS로 직접 연결되는 것
- 다른 리전에 있는 하나 이상의 VPC와 연결하고 싶다면 Direct Connect Gateway를 써야 함
...
- 게이트웨이에는 프라이빗 가상 인터페이스를 통해 가상 프라이빗 게이트웨이에 연결되어 있고, 첫 번째와 두 번째 리전 모두 같은 구성으로 되어 있음
- 설정을 통해 여러 VPC와 여러 리전을 연결할 수 있음
- Direct Connect 유형
	- 전용 연결 용량은 초당 1Gbp 혹은 10Gbp이 한도
		- 고객은 물리적 전용 이더넷 포트를 할당받음
		- 먼저 AWS에 요청을 보내면 AWS Direct Connect 파트너가 처리를 완료
	- 호스팅 연결도 가능한데 초당 50Mbp, 500Mbp에서 10Gbp까지 다양
		- AWS Direct Connect 파트너를 통해 또다시 연결을 요청
		- 전용 연결보다는 유연
		- 선택한 로케이션에서 초당 1, 2, 5, 10Gbp 이용이 가능하고,
	- 전용 연결이든 호스팅 연결이든새 연결을 만들려면 리드 타임이 한 달보다 길어질 때도 있음
	- 예를 들어 일주일 안에 빠르게 데이터를 전송하고 싶다면 Direct Connect는 답이 될 수 없음 -> 설치 기간이 한 달 정도 걸리기 때문
...
- 암호화 과정을 보자면 Direct Connect 로케이션을 가져와 해당 연결에 VPN 연결을 구축하면 Direct Connect에 암호화가 구성
- 기업 데이터 센터와 AWS 간 모든 트래픽을 암호화할 수 있음
- 복원력 및 아키텍처 모드 두 가지 모두 중요!
- 핵심 워크로드의 복원력을 높이기 위해서는 여러 Direct Connect를 설치하는 것이 좋음
- 기업 데이터 센터가 두 개 있고, Direct Connect 로케이션도 둘이라고 했을 때 중복이 발생 -> 프라이빗 VIF가 하나 있는데 다른 데 또 있다면 하나의 연결을 여러 로케이션에 수립한 것이므로 Direct Connect 하나가 망가져도 다른 하나가 예비로 남아 있기 때문에 복원력이 강해짐 -> 핵심 워크로드에 적합
- 핵심 워크로드 복원력을 최대로 끌어 올리고 싶다면 각 Direct Connect 로케이션에 독립적인 연결을 두 개씩 수립하면 복원력을 최대로 만들 수 있음
- 여기에서는 두 로케이션에 네 개의 연결을 수립해서 AWS에 연결
...
- Site to Site VPN을 백업 연결을 두어 기본 연결에 문제가 발생했을 때 이 연결을 사용하면 Site to Site VPN을 통해 퍼블릭 인터넷에 연결할 수 있으므로 안정성이 높아짐
...
- Transit Gateway로 복잡함을 해결할 수 있음
- 전이적 피어링 연결이 VPC 수천 개와 온프레미스 데이터 센터 Site-to-Site VPN, Direct Connect, 허브와 스포크 간 스타형 연결 사이에 생성
...
- Site-to-Site VPN과 VPN 연결을 선호한다면 고객 게이트웨이와 VPN 연결을 Transit Gateway에 연결할 수 있는데 Transit Gateway 일부로 모든 VPC에 액세스하는 것
- 네트워크 문제 해결에 유용
...
- Direct Connect Gateway 및 VPN 연결과 함께 작동하고 AWS에서 유일하게 IP 멀티캐스트를 지원하는 서비스로 IP 멀티캐스트가 나오면 Transit Gateway를 사용해야 함
- Site-to-Site VPN 연결 대역폭을 ECMP를 사용해 늘리는 경우에도 사용
...
- Transit Gateway에 VPC 네 개가 연결되어 있다고 가정 기업 데이터 센터는 Site-to-Site VPN으로 Transit Gateway에 연결
- Site-to-Site VPN 연결을 구축할 때 각기 앞쪽과 뒤쪽을 향하는 터널 두 개가 있고 Site-to-Site VPN을 VPC에 직접 연결하면 두 터널 모두 연결 하나로 사용되지만 Transit Gateway를 사용할 때는 두 터널이 동시에 사용
...
- Transit Gateway에 Site-to-Site VPN 연결을 더 추가할 수도 있습니다 두세 개 정도만 더해도 ECMP를 사용하면 처리량이 두세 배
...
- 관리 중인 보안 어플라이언스로 트래픽을 라우팅해
- 트래픽을 수집해 수집하려는 트래픽이 있는 소스 ENI를 정의
- 어디로 트래픽을 보낼지 대상을 정의 -> ENI나 네트워크 로드 밸런서 말이에요
- EC2 인스턴스와 탄력적 네트워크 인터페이스가 있고, ENI가 연결돼 문제없이 작동하며 EC2 인스턴스는 인터넷에 액세스 중
...
### 이그레스 전용 인터넷 Gateway
- 송신 전용 인터넷 게이트웨이는 IPv6 트래픽에만 사용되고 NAT Gateway와 비슷하지만 IPv6 전용임
- IPv4인 NAT Gateway와 다르게 IPv6에 사용됨
- VPC 인스턴스에서 IPv6상 아웃바운드 연결을 허용하고 동시에 인터넷이 인스턴스로 IPv6 연결을 시작하지 못하게 막는 기능 -> 라우팅 테이블 업데이트 필요
-  VPC에는 공용 및 사설 서브넷이 있다면 인터넷 게이트웨이를 통해 EC2 인스턴스가 인터넷에 액세스하고, 인터넷은 IPv6를 사용하여 인스턴스로 연결을 시작할 수 있음
- 인터넷 게이트웨이에 연결된 상태이고 공용 서브넷이기 때문
- 인터넷 게이트웨이가 없으니 서브넷이 비공개 상태로 송신 전용 인터넷 게이트웨이를 생성하면 사설 서브넷의 EC2 인스턴스가 송신 전용 인터넷 게이트웨이를 통해 IPv6로 인터넷에 액세스
- 인터넷에서 EC2 인스턴스까지 연결은 개시할 수 없음
- IPv6가 있는 VPC있을 때 공용 및 사설 서브넷이 있는데 둘 다 IPv4와 IPv6를 가짐
- 인터넷에 액세스하는 웹 서버는 인터넷 게이트웨이를 통해 IPv4와 IPv6로 액세스
- 공용 서브넷의 라우팅 테이블에서 로컬 IPv4와 로컬 IPv6 트래픽이 있고, 서브넷 또는 VPC의 CIDR 내부에 있음
...
- ::/0는 송신 전용 인터넷 게이트웨이 대상 ->IPv6 유형 트래픽 대상
###  VPC 요약
- CIDR은 IP 범위
- VPC는 가상 프라이빗 클라우드고 IPv4와 IPv6에 사용됨, 서브넷은 AZ에 연계되고, CIDR를 정의, 퍼블릭 서브넷과 프라이빗 서브넷이 있음
- 퍼블릿 서브넷은 인터넷 게이트웨이를 붙이고 서브넷에서 인터넷 게이트웨이로 가는 루트를 만들고 활성화되면IPv4와 IPv6 인터넷 액세스를 제공
- 루트 테이블을 편집해서 인터넷 게이트웨이, VPC 피어링 연결, VPC 엔드포인트 등으로 가는 루트를 갖게 됨
...
- 프라이빗 EC2 인스턴스에게 스케일링이 가능한 인터넷 액세스를 제공, IPv4만 사용할 수 있음
- NACL은 네트워크 ACL이고, 서브넷 수준에서 인바운드와 아웃바운드 액세스를 정의하는 방화벽 규칙, 안에는 임시 포트가 존재
- NACL은 상태가 없음, 그래서 인바운드와 아웃바운드 규칙이 항상 평가되는 반면에 보안 그룹 규칙은 상태를 갖고 있음
- 즉 인바운드가 허용되면 아웃바운드가 자동으로 허용되고, 그 반대로도 허용
- 보안 그룹 규칙은 EC2 인스턴스 수준에서 적용됨
- VPC 피어링은 2개의 VPC를 연결할 때 유용하, 중첩되지 않는 CIDR가 있을 경우에만 가능
- VPC 피어링 연결은 이행성이 없어서 3개의 VPC를 연결하려면  3개의 VPC 피어링 연결이 필요
- VPC 엔드포인트를 이용해서 우리는 Amazon S3, DynamoDB, CloudFormation 등 원하는 AWS 서비스에  VPC 안에서 프라이빗 액세스를 할 수 있음
- Amazon S3와 DynamoDB에 게이트웨이 엔드포인트가 있고, 나머지는 모두 인터페이스 엔드포인트
- VPC Flow Logs는 VPC 안의 모든 패킷에 관한 로그 수준 메타데이터를 얻는 방법
- 수락과 거부 트래픽에 관한 정보를 얻을 수 있음
- VPC Flow Logs는 VPC 서브넷 또는 ENI 수준에서 생성될 수 있고,  분석하고 Amazon S3로 전송해서 Athena를 이용해 분석할 수 있음, 또는 CloudWatch Logs로 전송해서 예를 들어 CloudWatch Log Insights를 이용해서 분석할 수 있음
- VPC를 데이터센터에 연결하는 방법은 두 가지
	- 사이트 간 VPN
		- 퍼블릭 인터넷을 통한 VPN 연결이
		- AWS에 가상 프라이빗 게이트웨이를 만들고 데이터센터에는 고객 게이트웨이를 만들어야 함
		- VPN 연결을 수립
		- 동일한 가상 프라이빗 게이트웨이를 사용해서 다수의 VPN 연결을 수립한다면 VPN CloudHub를 사용해서 허브 앤 스포크 모델로 사이트들을 연결할 수 있음
	- Direct Connect
		- 연결은 완전히 프라이빗 -> 퍼블릿 인터넷을 거치지 않지만 수립하는데 시간이 오래 걸림
		- 데이터센터를 Direct Connect 위치에 연결해야 함
		- 더 복잡하지만 더 안전하고 더 안정적
		- Direct Connect 게이트웨이는 다양한 AWS 리전에 있는 많은 VPC에 Direct Connect를 하기 위한 것
...
- 네트워크를 노출하지 않고도 VPC 안의 서비스를 수백 또는 수천 개의 고객 VPC에 노출할 수 있음
- ClassicLink는 EC2 클래식 인스턴스를 프라이빗하게 여러분이 VPC에 연결하기 위한 것
- Transit Gateway는 VPC, VPN, 그리고 Direct Connect를 위한 이행성 피어링 연결
- 모든 게 그걸 통과해서 흐를 수 있다는 점이 유리
- Traffic Mirroring은 ENI에서 다른 곳으로 네트워크 트래픽을 복사해서 추가로 분석
- VPC의 IPv6
	- 송신 전용 인터넷 게이트웨이 필요
...
- 사설 IP로 네트워크를 통해 서로 연결
- 다른 AZ에 있는 EC2 인스턴스까지 포함한다면 같은 리전 내 다른 두 AZ에 있는 EC2 인스턴스 두 개가 서로 소통하길 원할 경우 우선 공용 IP나 탄력적 IP를 사용하는 방법
	- 청구되는 비용은 GB당 2센트
...
- 인스턴스가 소통할 때 속도는 높이고 비용은 줄이려면 최대한 공용 IP보다는 **사설 IP**를 사용해야 한다는 점
- 가용 영역을 포함하는 리전이 하나 더 있는 경우 트래픽이 다른 리전으로 이동할 때 GB당 0.02달러가 청구
- **공용 IP보다는 사설 IP를 사용**
- 비용이 절약될 뿐만 아니라 네트워크 성능도 더 좋음
- 사설 네트워크에 자동으로 연결되기 때문
-  동일한 리전 내 가용 영역 두 곳에 있는 인스턴스 두 개가 통신할 때 공용 IP는 사용하지 않는 것을 추천
- 계산을 담당하는 클러스터가 있고, EC2 인스턴스 간에 활발히 소통하기를 요구한다면 최대한 비용을 절감하도록 같은 가용 영역을 사용해야 함
- 하지만 비용이 절감되는 만큼 가용성이 떨어짐
- 가용성과 비용 사이에서 균형을 유지해야 함
- RDS 데이터베이스가 있는데 읽기 전용 복제본을 생성하고, 분석을 진행하고 싶다는 등의 상황이면? -> 어떻게 하면 읽기 전용 복제본을 가장 저렴하게 생성할까?
...
- 송신 트래픽은 아웃바운드 트래픽 -> AWS에서 밖으로 나간다는 뜻
...
- 송신 트래픽 네트워크 비용을 최소로 줄이는 또 다른 방법은 Direct Connect를 사용할 경우
...
- S3 전송 가속화를 사용한다면 전송 속도가 50-500% 정도 빨라지는데 데이터 전송에 GB당 4-8센트가 청구 추가 비용이 발생
- 전송 가속화 기능은 비용이 듦
...
- Amazon S3 버킷 내 데이터를 이제 인터넷에서 액세스할 수 있음
...
- VPC 엔드 포인트를 갖추고 있다면 Amazon S3 버킷에 비공개로 데이터를 액세스할 수 있고, 다른 라우팅 테이블을 생성
...
### AWS 네트워크 파이어월
- 네트워크 액세스 제어 목록(NACL) Amazon VPC 보안 그룹 AWS WAF라는, HTTP를 통하여 특정 서비스에 유입되는 악성 요청을 막는 방화벽 DDoS를 막는 AWS Shield와 Shield Advanced가 있고, AWS Firewall Manager라는 여러 계정에 적용할 수 있는 WAF, Shield 등에 대한 규칙 관리자도 있음
...
- AWS Firewall Manager의 역할이라고 할 수 있음
- Network Firewall에서는 네트워크 트래픽을 세부적으로 관리할 수 있는데 VPC 수준에서 수천 개의 규칙을 지원하고, 수만 IP를 IP와 포트별로 필터링할 수 있음
