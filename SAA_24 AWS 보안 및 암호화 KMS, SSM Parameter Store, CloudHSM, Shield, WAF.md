### 암호화 101
- 전송 중 암호화(Encryption in flight)
	- TLS 또는 SSL이라고 하는데 TLS가 SSL 최신 버전
		- 데이터 전송 전에 암호화하고 받은 후 해독됨
		- 전송하는 이와 서버만이 암호화와 복호화를 하는 방법을 알고 있음
	...
	- 목표하는 대상 서버만이 전송하는 암호화된 데이터를 해독할 수 있음 -> 중간 서버는 해독 불가
- 서버 측 저장 중 암호화(Server-side encryption at rest)
	- 데이터가 암호화 되는 과정이 서버가 받은 후에 일어나고 데이터가 해독되는 것은 고객에게 되돌려 보내기 전에 진행됨
	- `데이터 키`를 주로 이용함
	- 서버는 데이터 키에 접근할 수 있어야 함
	- S3로 HTTP/HTTPS 객체를 보낼 때 서비스가 객체를 받고 해독함
		- 서비스는 데이터 키를 사용할 수 있고, 데이터 키와 해독된 객체를 사용해 그 객체를 저장 중 암호화할 수 있음
	- 이 객체를 다시 고객에게 전송할 때 암호화된 객체와 데이터 키가 함께 해독을 위해 사용되고 HTTPS로 고객에게 전송
	- 모든 암호화와 복호화가 서버에서 발생
- 클라이언트 측 암호화(Client-side encryption)
	- 클라이언트에서 데이터가 암호화되고 복호화되어 서버는 해독 불가
	- `매커니즘 봉투 암호화(leverage Envelope Encryption)`을 사용
	- 데이터 키를 가진 고객이 암호화 후 암호화된 객체를 받아서 FTP, S3, EBS 볼륨 등에 저장
	- 고객측에 암호화된 데이터를 주면 고객 측에서 데이터 키로 복호화
### KMS 개요
- AWS 서비스에서 암호화 = 대부분은 KMS 암호화
- KMS 서비스가 우리를 대신해서 암호화 키를 관리한다는 개념
- KMS는 승인에 관해 IAM과 완벽히 통합되어 있고, KMS로 암호화가 되어 있다면 데이터에 대한 액세스를 쉽게 제어 가능
- AWS KMS을 사용하면 CloudTrail을 통해 키를 사용하기 위해 이루어진 모든 API 호출을 감사할 수 있음
- KMS는 대부분의 AWS 서비스에서 원활하게 사용 가능
	- EBS 볼륨에 저장되어 있는 데이터를 암호화하고 싶으면 KMS 통합을 활성화하면 됨
	- S3, RDS, SSM 등도 같음
	- 개발자가 직접 KMS를 사용할 수 있는 것
- API 호출을 통해서도 KMS를 사용할 수 있음
	- AWS CLI나 SDK를 사용할 수 있어 모든 비밀을 KMS 키로 암호화할 수 있음
	- 그 암호화된 비밀을 코드나 환경 변수에 저장할 수 있음 -> 코드에 비밀 내용을 plain text로 저장지 않아도 됨
- KMS 키 = KMS 고객 마스터 키
- 대칭 KMS 키(Symmetric)
	- 데이터를 암호화하고 복호화하는 데 사용되는 `암호화 키가 하나`만 있다는 의미
...
- 비대칭 키(Asymmetric)
	...
	- AWS Cloud 외부에서 KMS API 키에 액세스할 수 없는 사용자에 의해 암호화 필요한 경우 퍼블릭 키를 써서 데이터를 암호화하고 그걸 개발자에게 전송. 그리고 계정 안에 있는 AWS의 프라이빗 키를 써서 그 데이터를 복호화
	- RSA & ECC key pairs
- KMS 키 안에는 다양한 타입의 KMS 키 존재
	...
	- 고객 관리형 키
		- 한달에 1달러의 비용이 발생
		- 가져오고 싶으면 임포트할 수 있음
		- KMS도 가격이 책정되는데  서비스에 대해 이루어진 API 호출 건에 대해 지불하게 됨
		- 대략 10,000회의 API 호출당 3센트
		- 자동 키 순환 : 키가 AMS 관리형 KMS 키라면 자동으로 1년마다 순환
		- KMS 안에서 생성한 고객 관리형 키라면 여러분이 자동 순환을 활성화해야 함
		- 1년마다 순환, 만일 임포트한 KMS 키라면 수동으로 순환시킬 수 있음
		- 별명을 사용 필요
-  KMS 키의 범위는 리전
	- KMS 키로 암호화된 EBS 볼륨이 eu-west-2에 있다면 다른 리전에 복사하기 위해서는 몇 가지 단계 필요
...
- 커스텀 KMS 키 정책
	...
	- 암호화된 스냅샷을 계정들 간에 복사하고 싶을 때 사용
		- 고객 관리형 KMS 키로 암호화된 스냅샷을 생성, 커스텀 키 정책을 첨부해야 되기 때문에 반드시 고객 관리형 KMS 키가 되어야 함, 그리고 교차 계정 액세스를 승인하기 위한 KMS 키 정책을 첨부
		- 암호화된 스냅샷을 타깃 계정과 공유하고 타깃 계정 안에서 원본 스냅샷의 사본을 생성, 다른 고객 관리형 키를 사용해서 그 타깃 계정 안에서 암호화 후 타깃 계정 안에서 스냅샷으로부터 볼륨을 생성할 수 있음
### KMS - Multi-Region Keys
- AWS KMS에는 다중 리전 키를 둘 수 있음
	- 선택된 한 리전에 기본 키를 갖는다는 의미
- us-east-1에 기본 키를 두면 다른 리전으로 복제 됨 
	- us-west-2, eu-west-1 ap-southeast-2로 복제
	- 유사해 보이는 것은 키 구성 요소가 복제되기 때문
...
- 하지만 KMS 다중 리전 키는 전역으로 사용할 수 없음
	- 기본 키가 있고 복제본이 있는 것
- 각 다중 리전 키는 자체 키 정책 등으로 각각 독립적으로 관리
- 특정 사용 사례를 제외하고는 다중 리전 키 사용을 권장하지 않음 
	- KMS 키는 단일 리전에 제한되는 것을 선호
- 다중 리전 키의 사용 사례
	- 전역 클라이언트 측 암호화
	...
	- 중요한 것은 전체 테이블만 암호화하는 게 아니라 저장 데이터 암호화이므로 테이블의 속성을 암호화하여 특정 클라이언트만 사용할 수 있게 됨
		- 데이터베이스 관리자도 사용할 수 없음
	- 이때 Amazon DynamoDB 암호화 클라이언트를 사용
		- us-east-1과 ap-southeast-2가 있을 때  us-east-1의 KMS 다중 리전 키는 다른 리전인 ap-southeast-2에 복제
		- 클라이언트 애플리케이션에서 DynamoDB에 데이터를 삽입하려면 먼저 속성을 암호화해야 함
		- 다중 리전 키를 사용해 암호화할 속성을 암호화
		- DynamoDB 테이블 필드는 클라이언트 측 암호화가 되지 않지만 사회 보장 번호 등은 암호화될 것
		- DynamoDB 테이블에 액세스할 수 있는 데이터베이스 관리자가 '사회 보장 번호' 속성을 암호화하는 데 사용한 KMS 키에 액세스할 수 있는 권한이 없다면 액세스할 수 없음
			- 데이터베이스 관리자로부터도 보호할 수 있음
	- DynamoDB 테이블이 전역 테이블인 경우 해당 테이블의 데이터는 다른 리전인 ap-southeast-2로 복제
...
### SSM 매개변수 저장소(SSM Parameter Store) 개요
...
- CloudFormation과 통합 가능
	...
	- SSM Parameter Store가 KMS로 구성을 암호화 함
...
- Systems Manager에는 두 가지 매개변수 티어가 있는데 바로 표준(Standard)과 고급(Advanced)
	- 큰 차이는 용량으로 각각 4KB, 8KB
- 매개 변수 정책 적용 여부는 표준 티어에선 적용되지 않고 고급 티어에선 적용됨
...
- <u>Secrets Manager는 X일마다 강제로 암호를 교체하는</u> 기능이 있어 암호 관리를 더 효율적
	- AWS Secrets Manager로 교체할 암호를 강제 생성 및 `자동화`할 수 있음
- 새 암호를 생성할 Lambda 함수를 정의해야 함
- AWS Secrets Manager는 AWS가 제공하는 다양한 서비스와도 통합됨
...
- 다중 리전 암호의 개념
	+ 복수 AWS 리전에 암호를 복제할 수 있고, AWS Secrets Manager 서비스가 기본 암호와 동기화된 읽기 전용 복제본을 유지
	- 두 리전이 있을 때 기본 리전에 암호를 하나 만들면 보조 리전에 <u>동일한 암호가 복제됨</u>
...
- 오토 스케일링 그룹에 연결된 ALB가 있다고 하면 이때 <u>애플리케이션 로드 밸런서(ALB)를 HTTPS 엔드 포인트로서 노출하려 함</u>
...
- 퍼블릭 인증서는 어떤 과정을 통해 요청?
	- 인증서에 포함할 도메인 이름을 나열
		...
		+ 도메인 수에는 제한이 없음
	- 유효성 검증 방법인 <u>DNS 검증과 이메일 검증</u> 중 어느 것을 거칠지 선택
		+ SSL 인증서를 자동 갱신하려면 DNS 검증을 쓰는 편이 유리
		...
		- ACM에서 스스로 생성된 모든 인증서를  만료 60일 전에 자동으로 갱신함
- ACM에서 퍼블릭 인증서는 어떻게 가져오면 될까?
	...
	- EventBridge에서 <u>Lambda 함수나 SNS 주제 또는 SQS 대기열을 트리거하거나 AWS Config을 사용</u>
		- acm-certificate-expiration-check 라는 관리형 규칙이 있는데 만료된 인증서를 확인하는 규칙인데 여기서 일수를 조정할 수도 있음
		- Config 서비스에 규칙을 설정하면 Config 서비스가 ACM 서비스를 검사해서 규칙에 위배되는 인증서가 있을 경우 규정 비준수 이벤트가 EventBridge로 전송되고 Lambda나 SNS, SQS를 트리거
...
- ACM이 API Gateway와 어떻게 통합되는지?
	- 엔드 포인트 유형
		+ 엣지 최적화(edge-optimized) 유형 
			+ 글로벌 클라이언트를 위한 유형으로 CloudFront 엣지 로케이션으로 요청을 라우팅하여 지연을 줄이는 방법 
			+ 하나의 리전에만 있는 API Gateway로 보내지는 경우
		+ 리전(regional) 엔드 포인트 유형
			+ 클라이언트가 API Gateway와 같은 리전에 있을 때를 쓰임
			- CloudFront는 사용할 수 없지만 자체 CloudFront 배포를 생성하여 캐싱 및 배포 전략을 제어할 수는 있음
	...
	- <u>ACM은 엣지 최적화 및 리전 엔드포인트에 적합</u>
	- ACM을 API Gateway와 통합하려면 우선 API Gateway에 사용자 지정 도메인 이름이라는 리소스를 생성해야 함
...
- 웹 액세스 제어 목록(ACL)과 규칙을 정의해야 함
	...
	- 속도 기반 규칙을 설정하면 IP당 요청 수를 측정하여 디도스 공격을 막을 수도 있음
		+ ex: 특정 IP에서 초당 11개 이상의 요청을 보내지 못하게 함
...
- 디도스란 분산 서비스 거부 공격이라는 뜻으로 인프라에 동시에 엄청난 양의 요청이 세계 곳곳의 여러 컴퓨터에서 유입되는 공격
	- 목적은 인프라에 과부하를 일으키는 것으로 실제 사용자들에게 서비스를 제공할 수 없게 만듦
	+ 분산 서비스 거부가 일어나는 것
...
- 고급 보호가 필요한 고객을 위한 AWS Shield advanced 서비스도 있는데 어드밴스드는 선택적으로 제공되는 디도스 완화 서비스로 조직당 월 3,000달러를 지불
...
- 또 다른 아키텍처로 CloudFront와 API Gateway가 있다고 가정
- CloudFront는 엣지 로케이션에서 사용하므로 엣지 로케이션 완화임
...
- Global Accelerator는 Shield와 통합되어 CloudFront가 백엔드와 호환되지 않는 경우 DDoS 공격 방어에 유용하게 쓰임
...
- DDoS 완화 모범 사례
	...
	- EC2 인스턴스에 관리 가능한 양의 트래픽이 들어오면서 오토 스케일링 그룹이 이에 따라 확장 가능
...
- GuardDuty는 CloudTrail Event Logs 같은 많은 입력 데이터를 확인해서 비정상적인 API 호출이나 무단 배포를 검색
	...
	+ 비정상적인 IP 주소를 검색 해 , DNS 로그의 경우엔 DNS 쿼리 안에서 인코딩된 데이터를 전송하는 EC2 인스턴스를 검색하고 인스턴스에 문제가 있다는 것을 확인
	+ 그리고 옵션 기능이 있어서 EKS 감사 로그나 RDS 및 Aurora 로그인 이벤트, EBS, 람다, S3 데이터 이벤트 등 다른 입력 데이터 소스를 검색할 수 있음
...
### Amazon Inspector
- Amazon Inspector는 자동화된 보안 평가를 실행할 수 있는 서비스
...
- Amazon Inspector는 무엇을 평가하는 것?
	- Inspector는 <u>실행 중인 EC2 인스턴스, Amazon ECR의 컨테이너 이미지, Lambda 함수</u>에만 사용됨
	- 필요할 때 인프라만 지속적으로 스캔
