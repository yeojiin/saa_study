...
- 버킷은 계정 안에 생성되고 **전역적으로 고유한 하나의 이름이어야 함**
- 버킷은 리전 수준에서 정의됨
...
- 사용자 기반: IAM정책 필요
- 리소스 기반: 
	- Bucket Policies: S3 콘솔에 직접 할당하는 전체 버킷 규칙 -> S3 버켓에 액세스 할 수 있는 교차 계정(cross account)
	- Object Access Control List(ACL)
	- Bucket Access Control List(ACL)
- IAM원칙이 특정 API 호출 시 S3 객체에 액세스 할 수 있음
- 암호화
- 버킷 정책
	-  json으로 작성됨
	- Resource:  적용되는버킷과 객체를 나타
...
- 종류
	...
	- glacier instant retrieval
		- Glacier은 콜드 스코리지이고 비용은 스토리지 비용과 검색 비용이 발생
		- 최소 보관 기간이 90일이지만 밀리초 이내에 액세스 해야 하는 경우
		- 데이터 검색이 `즉각적`
	- glacier Flexible retrieval
		- glacier instant retrieval 같은데 데이터 검색 시 최대 12시간까지 걸림
